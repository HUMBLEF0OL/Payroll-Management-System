 ...
 * @author HUMBLEFOOL
 */
// here we have to use ItemListener as we are updating the values on selecting a item
public class Update_salary extends JFrame implements ActionListener, ItemListener {
	...
	Update_salary() {
		...
		try {
			...
			// fetching the data from salary table and storing it in a resultSet
			while (rs.next()) {
		...
		// the layout is null here so we have to manually set the location of each item
		// here
		...
		// we have to add ItemListener on choices
	...
	public void actionPerformed(ActionEvent ae) {
		// if button 1 is select then do the below
		// getSource is holds the button which is selected
		if (ae.getSource() == b1) {
			...
			String qry = "update salary set hra=" + hra + ",da=" + da + ",med=" + med + ",pf=" + pf + ",basic_salary="
					+ basic + "  where id=" + c2.getSelectedItem();
